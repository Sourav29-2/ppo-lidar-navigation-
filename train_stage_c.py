...
def run_fine_tuning():
    ...
    start_path = checkpoints_dir / "oscillation_latest.pt"
    ...
    while ft_steps < TOTAL_FT_STEPS:
        ...
        if update_idx % save_every_updates == 0 or ft_steps >= TOTAL_FT_STEPS:
            ...
            torch.save(ckpt_data, checkpoints_dir / "hybrid_curriculum_latest.pt")
            print(f"[{ft_steps}/{TOTAL_FT_STEPS}] Saved hybrid_curriculum_latest.pt")
            
            # Custom milestone saves for curriculum
            if ft_steps in [20000, 40000, 60000, 80000, 100000]:
                milestone = ft_steps // 1000
                torch.save(ckpt_data, checkpoints_dir / f"hybrid_curriculum_{milestone}k.pt")
                print(f"[{ft_steps}/{TOTAL_FT_STEPS}] Saved hybrid_curriculum_{milestone}k.pt")
