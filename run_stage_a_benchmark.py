"""Stage A Comparative Benchmark Script."""
...
def run_episode(env: BenchmarkEnv, actor: nn.Module, scenario: dict, model_name: str, device: torch.device):
    ...
    res = {
        ...
        "stagnation_rate": stagnation_events/steps if steps>0 else 0,
...
def main():
    ...
    models = {
        ...
        "Stage-A": PROJECT_ROOT / "checkpoints" / "stagnation_16k.pt"
    }
    ...
    with open("diagnostics/stage_A_champion_comparison.csv", "w", newline="") as f:
        ...
        metrics = ["success", "collision", "timeout", "path_length", "episode_time", "minimum_clearance", "stagnation_events"]
        for m in metrics:
            c_val = _mean([r[m] for r in c_rows]) if m != "stagnation_events" else _sum([r[m] for r in c_rows])
            f_val = _mean([r[m] for r in f_rows]) if m != "stagnation_events" else _sum([r[m] for r in f_rows])
    ...
    c_stag = _sum([r["stagnation_events"] for r in c_rows])
    f_stag = _sum([r["stagnation_events"] for r in f_rows])
...
3. Does Stage-A reduce stagnation?
...
8. Is stagnation_16k.pt actually better than best_success.pt?
