...
Rules (enforced in this file):
  • Loads from hybrid_phase4/best_success.pt — NEVER overwrites the original Champion files.
  • Saves ONLY to checkpoints/hybrid_phase5/ (isolated directory).
  • Hard stop at exactly PHASE5_TOTAL_STEPS (50,000). No auto-extension.
...
# ─────────────────────────────────────────────────────────────────────────────
PHASE5_TOTAL_STEPS   = 50_000   # hard stop — no auto-extension
PHASE5_EVAL_INTERVAL = 4_096    # evaluate every ~1 rollout (~5k steps effective)
PHASE5_ROLLOUT_SIZE  = 4_096
PHASE5_EVAL_EPISODES = 50       # deterministic benchmark (same as all previous models)
PHASE5_WALL_EVAL_EPS = 15       # additional wall-blocked-specific episodes for sub-metrics
PHASE5_SEED          = 42       # same eval seed as all previous models

# Source Champion checkpoint (read-only — never overwritten)
CHAMPION_CKPT = PROJECT_ROOT / "checkpoints" / "hybrid_phase4/best_success.pt"

# Phase 4 output directory (isolated — never touches originals)
PHASE5_DIR = PROJECT_ROOT / "checkpoints" / "hybrid_phase5"

# Phase 4 training log
PHASE5_LOG  = PROJECT_ROOT / "checkpoints" / "hybrid_phase5" / "training_log.csv"
...
# ─────────────────────────────────────────────────────────────────────────────
def generate_eval_goals(num_episodes: int = PHASE5_EVAL_EPISODES) -> list[np.ndarray]:
    rng = np.random.default_rng(PHASE5_SEED)
...
def generate_wall_eval_goals(num: int = PHASE5_WALL_EVAL_EPS) -> list[np.ndarray]:
...
def train_phase5() -> None:
    ...
    print(f"  Total steps  : {PHASE5_TOTAL_STEPS:,}")
    print(f"  Eval interval: every ~{PHASE5_EVAL_INTERVAL:,} steps")
    print(f"  Source ckpt  : {CHAMPION_CKPT.name}")
    print(f"  Output dir   : {PHASE5_DIR}")
    print(f"  Curriculum   : 40% wall-blocked / 30% complex / 30% random")
    print(f"{'='*60}\n")

    # ── 2. Isolate Champion checkpoint ────────────────────────────────────────
    PHASE5_DIR.mkdir(parents=True, exist_ok=True)
    seed_ckpt_path = PHASE5_DIR / "seed_champion.pt"
    ...
    buffer = RolloutBuffer(rollout_size=PHASE5_ROLLOUT_SIZE, observation_dim=obs_dim,
    ...
    # ── 4. TensorBoard ───────────────────────────────────────────────────────
    writer = SummaryWriter(log_dir=str(PROJECT_ROOT / "runs" / "hybrid_phase5"))
    ...
    ckpt = load_checkpoint(PHASE5_DIR, current_config=CHECKPOINT_CONFIG)
    if ckpt is not None:
        print("  [ckpt] Resuming existing Phase 4 run...", flush=True)
    else:
        # First run — load from isolated seed copy
        ckpt = load_checkpoint(PHASE5_DIR / "..", current_config=None)  # don't auto-pick
    ...
    existing = load_checkpoint(PHASE5_DIR, current_config=CHECKPOINT_CONFIG)
    if existing is not None:
        ...
        if total_steps >= PHASE5_TOTAL_STEPS:
    ...
    def _shutdown(signum, frame):
        ...
        try:
            ...
            save_checkpoint(ckpt_e, PHASE5_DIR)
    ...
    print(f"\n{'='*60}")
    print(f"TRAINING START — target: {PHASE5_TOTAL_STEPS:,} steps")
    print(f"{'='*60}\n")
    ...
    for k, v in _eval_metrics.items():
        writer.add_scalar(f"Phase5/{k}", v, global_step=total_steps)

    next_eval_at = PHASE5_EVAL_INTERVAL

    while total_steps < PHASE5_TOTAL_STEPS:
        # ── Collect one rollout ───────────────────────────────────────────────
        for _ in range(PHASE5_ROLLOUT_SIZE):
            total_steps += 1
        ...
        returns_np = buffer.returns[:PHASE5_ROLLOUT_SIZE].cpu().numpy()
        values_np  = buffer.values[:PHASE5_ROLLOUT_SIZE].cpu().numpy()
        ...
        # TensorBoard
        writer.add_scalar("Phase5/Loss/actor",      ppo_metrics["actor_loss"],  total_steps)
        writer.add_scalar("Phase5/Loss/critic",     ppo_metrics["critic_loss"], total_steps)
        writer.add_scalar("Phase5/Policy/entropy",  ppo_metrics["entropy"],     total_steps)
        writer.add_scalar("Phase5/Value/expl_var",  expl_var,                   total_steps)
        writer.add_scalar("Phase5/Reward/avg",      avg_rew,                    total_steps)
        writer.add_scalar("Phase5/Curriculum/wall_blocked",
                          class_counts.get("wall_blocked", 0), total_steps)

        print(
            f"[Update {update_idx:04d}] Steps: {total_steps:,}/{PHASE5_TOTAL_STEPS:,} | "
        ...
        save_checkpoint(ckpt_out, PHASE5_DIR)

        # ── Periodic evaluation ───────────────────────────────────────────────
        if total_steps >= next_eval_at or total_steps >= PHASE5_TOTAL_STEPS:
            ...
            if eval_m["success_rate"] > best_p4_success:
                ...
                save_checkpoint(ckpt_best, PHASE5_DIR, is_best=True)
                print(f"  [Phase5] New best success rate: {best_p4_success:.1f}% "
                      f"→ hybrid_phase5/best_success.pt", flush=True)
    ...
    save_checkpoint(ckpt_final, PHASE5_DIR, is_final=True)
    ...
    print(f"  Wall bypass successes     : {int(best_p4_metrics.get('wall_bypass_successes', 0))}/"
          f"{PHASE5_WALL_EVAL_EPS}", flush=True)
    print(f"\n  Output directory          : {PHASE5_DIR}", flush=True)
    print(f"  Training log              : {PHASE5_LOG}", flush=True)
