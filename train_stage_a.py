"""Stage A — Stagnation Penalty Experiment (50k steps)"""
...
def run_fine_tuning():
    ...
    writer = SummaryWriter(log_dir=str(PROJECT_ROOT / "runs" / "stage_a_stagnation"))
    ...
    while ft_steps < TOTAL_FT_STEPS:
        ...
        writer.add_scalar("StageA/actor_loss", metrics["actor_loss"], global_step=ft_steps)
        writer.add_scalar("StageA/critic_loss", metrics["critic_loss"], global_step=ft_steps)
        writer.add_scalar("StageA/learning_rate", current_lr, global_step=ft_steps)
        writer.add_scalar("StageA/stagnation_penalty_total", sum(stagnation_penalties), global_step=ft_steps)
        ...
        if update_idx % save_every_updates == 0 or ft_steps >= TOTAL_FT_STEPS:
            ...
            ckpt_path = checkpoints_dir / f"stagnation_{ckpt_label}.pt"
            torch.save(ckpt, ckpt_path)
            torch.save(ckpt, checkpoints_dir / "stagnation_latest.pt")
